...
`.geojson` files, according to the GIS leader ESRI, are an [open standard geospatial data interchange format](https://doc.arcgis.com/en/arcgis-online/reference/geojson.htm) that represents simple geographic features and their non-spatial attributes. GeoJSON is based on the JavaScript Object Notation (JSON) file format which is a lightweight data exchange format that is easily interpretable by both man and machine. In very few instances does any data format please both sides of the divide but JSON does, and this [site](https://www.oracle.com/ke/database/what-is-json/) provides examples. Anyway, just like you can tell from the name, JSON is based on the JavaScript programming language. If you have worked with JavaScript before, it looks very much like a data format based on dictionaries. In essence, JSON is a large dictionary holding other *dictionaries* of data within it. A GeoJSON is a JSON file that follows a certain structure and has spatial index and geometry specifications in it. See this [website](https://datascience.aero/geospatial-data-representation-the-geojson-format/) for a GeoJSON example. 
...
It may look intimidating to create a GeoJSON file without hardly making any errors, and it actually is, but luckily the [geojson.io](https://geojson.io/#map=2/0/20) website does the heavy lifting for us. We shall head over to it and create a `geojson` file of some cities and their population. 
...
Okay, next step. Once the server beams back the data to us, what do we do with it? According to [Digital Ocean](https://www.digitalocean.com/community/tutorials/js-fetch-api), the response is not *actually* the data in the original format but rather *a series of methods that can be used depending on what you want to do with the information*. This sounds downright confusing. Nevertheless, it is *at least* crystal clear that the end goal is to have our fetched data in JSON format. Converting an object to JSON is done using the `json()` method.  We parse `.json()` to a `then` method which is an asynchronous function^[Asynchronous programming is a technique that enables your program to start a potentially long-running task and still be able to be responsive to other events while that task runs, rather than having to wait until that task has finished. Once that task has finished, your program is presented with the result. See [Mozilla](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Asynchronous/Introducing)]. After the `fetch` function is successful, the object is stored in the `response` argument, which is in brackets, and then the function returns the result as a `json` object. 
...
Here are the [full files](https://github.com/sammigachuhi/my-leaflet-project/tree/main/my-leaflet-code/chapter-5-geojson-leaflet) used in this chapter.
...
- The [geojson.io](https://geojson.io/#map=2/0/20) website allows a user to create a GeoJSON file interactively and intuitively.
